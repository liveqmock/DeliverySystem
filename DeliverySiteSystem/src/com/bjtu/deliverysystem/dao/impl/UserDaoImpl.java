...
public class UserDaoImpl extends HibernateDaoSupport implements UserDao {
    ...
    @SuppressWarnings("unchecked")
	public Admin isExist(User user) {
        System.out.println("UserDaoImpl开始查找");
        List user_list = getHibernateTemplate().find("from Admin u where u.account=? and u.pwd = ?", user.getNickname(), user.getPwd());
        if (user_list.size() > 0) {
            System.out.println(((Admin) user_list.get(0)).getAccount());
            return (Admin) user_list.get(0);
        } else {
            return null;
        }
    }
