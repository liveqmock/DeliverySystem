...
function setLevel(n)
{
	var level = n;
	if(level==1){
		document.all.selCity.disabled=true;
	}else if(level==2){
		document.all.selCity.disabled=false;
	}else if(level==3){
		document.all.selCity.disabled=false;
	}
}
                ...
                <!--HTML部分-->
              <form name="add" action="addDispatchSiteAction">
                <br/>
                <label for="NO">编 号：</label>
                <input name="NO" type="text"/>
                <br>
                <label for="level">级 别：</label>
                <select name="level" onChange = "setLevel(this.options[this.selectedIndex].value)">
        			<option value=1>一级</option>
        			<option value=2>二级</option>
        			<option value=3>三级</option>
       			 </select>
       			 <br>
                <label for="province" style="width:200px">省/市：</label>
                <select name="province" id="selProvince" onChange = "getCity(this.options[this.selectedIndex].value)" >  
                </select>
                <br/>  
                <label for="city" style="width:200px">市/区：</label>
